...
def parse_args():
    parser = argparse.ArgumentParser(
        description="""Proxy Tester for Pentesting / Ethical Hacking

Proxy list file example (proxy_list.txt):
...
""",
    ...
    parser.add_argument("--timeout", type=int, default=5, help="Request timeout in seconds (default 5). Will retry once with double timeout if it fails")
        ...
        if args.output:
            with open(args.output, "w") as f:
                for proxy in working_proxies:
                    f.write(proxy + "\n")
            print(f"{Fore.GREEN}Working proxies saved to {args.output}{Style.RESET_ALL}")
        else:
